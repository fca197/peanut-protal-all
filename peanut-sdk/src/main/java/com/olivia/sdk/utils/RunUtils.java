...
public class RunUtils implements AutoCloseable {
  ...
  private static final ExecutorService platformExecutor = Executors.newThreadPerTaskExecutor(
      Thread.ofPlatform().name("platform-task-", 1).priority(Thread.NORM_PRIORITY).group(new ThreadGroup("platform")).factory());
  ...
  public static <T extends Runnable> boolean run(String key, T runnable, Duration timeOut) {
    return run(key, List.of(runnable), timeOut);
  }
   ...
   * @return 所有任务是否都执行成功
   */
  public static boolean run(String key, List<? extends Runnable> runnableList, CallBackRunnable callBackRunnable) {
    return run(key, runnableList, true, callBackRunnable, Duration.ofMinutes(5));
  }
  ...
  public static boolean run(String key, List<? extends Runnable> runnableList, Boolean callOnException, CallBackRunnable callBackRunnable, Duration timeout) {
    ...
    try {
      ...
      if (!allCompleted) {
        failedCount.incrementAndGet();
//        log.warn("任务执行超时 - key: {}, 任务数 :{} 超时时间: {}ms, 未完成任务数: {}", key, runnableList.size(), timeout.toMillis(), countDownLatch.getCount());
//        return false;
      }
...
//      return false;
    ...
    log.info("任务执行完成 - key: {}, 总任务数: {}, 成功数: {},  超时时间: {}ms, 失败数: {} 未完成任务数:  {}", key, taskCount, taskCount - failedCount.get(),
        timeout.toMillis(), failedCount.get(), countDownLatch.getCount());
