...
@Transactional
public class ApsMakeCapacityGoodsServiceImpl extends MPJBaseServiceImpl<ApsMakeCapacityGoodsMapper, ApsMakeCapacityGoods> implements
    ApsMakeCapacityGoodsService {
  ...
  public @Override ApsMakeCapacityGoodsQueryListRes queryList(ApsMakeCapacityGoodsQueryListReq req) {
    ...
    List<ApsMakeCapacityGoodsDto> dataList = list.stream()
        .map(t -> $.copy(t, ApsMakeCapacityGoodsDto.class)).collect(Collectors.toList());
  ...
  public @Override DynamicsPage<ApsMakeCapacityGoodsExportQueryPageListInfoRes> queryPageList(ApsMakeCapacityGoodsExportQueryPageListReq req) {
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<ApsMakeCapacityGoodsExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, ApsMakeCapacityGoodsExportQueryPageListInfoRes.class));
    ...
    List<ApsMakeCapacityGoodsExportQueryPageListInfoRes> listInfoRes = $.copyList(records,
  ...
  public ApsMakeCapacityGoodsInsertRes save(ApsMakeCapacityGoodsInsertReq req) {
    Map<String, ApsMakeCapacityGoods> apsMakeCapacityFactoryMap = this.list(
            new LambdaQueryWrapper<ApsMakeCapacityGoods>().eq(ApsMakeCapacityGoods::getGoodsId,
                req.getGoodsId())).stream()
...
//    Map<String, ApsMakeCapacityFactory> apsMakeCapacityFactoryMap = new HashMap<>();
    ...
    for (int i = 0; i < size; i++) {
      ...
      localDateBetween.forEach(currentDate -> {
        ...
        ApsMakeCapacityGoods makeCapacityGoods = apsMakeCapacityFactoryMap.getOrDefault(key,
            new ApsMakeCapacityGoods());
        makeCapacityGoods.setFactoryId(req.getFactoryId()).setYear(currentDate.getYear())
            .setMonth(currentDate.getMonthValue()).setGoodsId(req.getGoodsId());
        for (int j = cuIndex + 1; j < size; j++) {
          ...
          if (dateOptional.isPresent()) {
            ReflectUtil.setFieldValue(makeCapacityGoods,
                getField(ApsMakeCapacityGoods.class, "dayMin" + currentDate.getDayOfMonth()),
                nextMakeCapacityConfig.getMinValue());
            ReflectUtil.setFieldValue(makeCapacityGoods,
                getField(ApsMakeCapacityGoods.class, "dayMax" + currentDate.getDayOfMonth()),
    ...
    List<ApsMakeCapacityGoods> updateList = entityList.stream()
        .filter(t -> Objects.nonNull(t.getId())).toList();
    if (CollUtil.isNotEmpty(updateList)) {
      this.updateBatchById(updateList);
    }
    List<ApsMakeCapacityGoods> insertList = entityList.stream()
  ...
  public @Override void setName(
      List<? extends ApsMakeCapacityGoodsDto> apsMakeCapacityGoodsDtoList) {
    ...
    if (Objects.nonNull(obj)) {
      q.eq(Objects.nonNull(obj.getFactoryId()), ApsMakeCapacityGoods::getFactoryId,
          ...
          .eq(Objects.nonNull(obj.getMakeCapacityQuantity()),
              ApsMakeCapacityGoods::getMakeCapacityQuantity, obj.getMakeCapacityQuantity())
          .eq(Objects.nonNull(obj.getYear()), ApsMakeCapacityGoods::getYear, obj.getYear())
          .eq(Objects.nonNull(obj.getMonth()), ApsMakeCapacityGoods::getMonth, obj.getMonth());
    }
