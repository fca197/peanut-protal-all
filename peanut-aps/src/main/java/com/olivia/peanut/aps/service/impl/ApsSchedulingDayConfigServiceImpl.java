...
@Transactional
public class ApsSchedulingDayConfigServiceImpl extends MPJBaseServiceImpl<ApsSchedulingDayConfigMapper, ApsSchedulingDayConfig> implements
    ApsSchedulingDayConfigService {
  ...
  public @Override ApsSchedulingDayConfigQueryListRes queryList(ApsSchedulingDayConfigQueryListReq req) {
    ...
    List<ApsSchedulingDayConfigDto> dataList = list.stream()
        .map(t -> $.copy(t, ApsSchedulingDayConfigDto.class)).collect(Collectors.toList());
  ...
  public @Override DynamicsPage<ApsSchedulingDayConfigExportQueryPageListInfoRes> queryPageList(ApsSchedulingDayConfigExportQueryPageListReq req) {
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<ApsSchedulingDayConfigExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, ApsSchedulingDayConfigExportQueryPageListInfoRes.class));
    ...
    List<ApsSchedulingDayConfigExportQueryPageListInfoRes> listInfoRes = $.copyList(records,
        ApsSchedulingDayConfigExportQueryPageListInfoRes.class);
  ...
  public @Override void setName(
      List<? extends ApsSchedulingDayConfigDto> apsSchedulingDayConfigDtoList) {
    ...
    if (CollUtil.isNotEmpty(apsSchedulingDayConfigDtoList)) {
      ...
      apsSchedulingDayConfigDtoList.forEach(t -> t.setSchedulingDayConfigItemDtoList(
          $.copyList(itemMap.get(t.getId()), ApsSchedulingDayConfigItemDto.class)));
  ...
  private MPJLambdaWrapper<ApsSchedulingDayConfig> getWrapper(ApsSchedulingDayConfigDto obj) {
    ...
    if (Objects.nonNull(obj)) {
      q.eq(Objects.nonNull(obj.getId()), ApsSchedulingDayConfig::getId, obj.getId())
          .eq(Objects.nonNull(obj.getFactoryId()), ApsSchedulingDayConfig::getFactoryId,
              obj.getFactoryId())
          .eq(Objects.nonNull(obj.getProcessId()), ApsSchedulingDayConfig::getProcessId,
              obj.getProcessId())
          .eq(StringUtils.isNoneBlank(obj.getSchedulingDayNo()),
              ApsSchedulingDayConfig::getSchedulingDayNo, obj.getSchedulingDayNo())
          .eq(StringUtils.isNoneBlank(obj.getSchedulingDayName()),
              ApsSchedulingDayConfig::getSchedulingDayName, obj.getSchedulingDayName())
          .eq(Objects.nonNull(obj.getIsDefault()), ApsSchedulingDayConfig::getIsDefault,
              obj.getIsDefault())
