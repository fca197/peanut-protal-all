...
public class ApsGoodsSaleProjectConfigServiceImpl extends MPJBaseServiceImpl<ApsGoodsSaleProjectConfigMapper, ApsGoodsSaleProjectConfig> implements
  ...
  final static Cache<Long, Map<Long, List<ApsGoodsSaleProjectConfig>>> apsGoodsSaleProjectConfigCache = CacheBuilder.newBuilder()
      .maximumSize(100)
      .expireAfterWrite(TIME_OUT, TimeUnit.MINUTES).build();
  final static Cache<String, Map<String, ApsSaleConfig>> saleConfigSaleCodeCache = CacheBuilder.newBuilder()
      .maximumSize(100).expireAfterWrite(TIME_OUT, TimeUnit.MINUTES).build();
  final static Cache<String, Map<Long, ApsSaleConfig>> saleConfigIdCache = CacheBuilder.newBuilder()
      .maximumSize(100).expireAfterWrite(TIME_OUT, TimeUnit.MINUTES).build();

  final static Cache<String, Map<Long, ApsProjectConfig>> projectConfigCache = CacheBuilder.newBuilder()
      .maximumSize(100).expireAfterWrite(TIME_OUT, TimeUnit.MINUTES).build();
  ...
  public @Override ApsGoodsSaleProjectConfigQueryListRes queryList(ApsGoodsSaleProjectConfigQueryListReq req) {
    ...
    List<ApsGoodsSaleProjectConfigDto> dataList = list.stream()
        .map(t -> $.copy(t, ApsGoodsSaleProjectConfigDto.class)).collect(Collectors.toList());
  ...
  public @Override DynamicsPage<ApsGoodsSaleProjectConfigExportQueryPageListInfoRes> queryPageList(ApsGoodsSaleProjectConfigExportQueryPageListReq req) {
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<ApsGoodsSaleProjectConfigExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, ApsGoodsSaleProjectConfigExportQueryPageListInfoRes.class));
    ...
    List<ApsGoodsSaleProjectConfigExportQueryPageListInfoRes> listInfoRes = $.copyList(records,
  ...
  public ApsGoodsSaleProjectConfigSale2ProjectRes sale2project(ApsGoodsSaleProjectConfigSale2ProjectReq req) {
    ...
    Map<String, ApsSaleConfig> apsSaleConfigMap = saleConfigSaleCodeCache.get("",
        () -> this.apsSaleConfigService.list().stream()
            .collect(Collectors.toMap(ApsSaleConfig::getSaleCode, Function.identity())));

    Map<Long, ApsSaleConfig> saleConfigIdMap = saleConfigIdCache.get("",
        () -> this.apsSaleConfigService.list().stream()
            .collect(Collectors.toMap(ApsSaleConfig::getId, Function.identity())));
    ...
    Map<Long, ApsProjectConfig> apsProjectConfigMap = projectConfigCache.get("",
        () -> this.apsProjectConfigService.list().stream()
            .collect(Collectors.toMap(ApsProjectConfig::getId, Function.identity())));

    List<String> saleConfigList = Arrays.stream(req.getSaleConfig().split(",")).toList();
    String str = saleConfigList.stream().filter(t -> !apsSaleConfigMap.containsKey(t))
    ...
    IntStream.range(0, req.getConvertCount().intValue()).forEach(index -> {
      ...
      saleConfigList.forEach(saleCode -> {
        ...
        long count = goodsSaleProjectConfigList.stream()
            .filter(t -> t.getCurrentIndex() > t.getQuantity()).count();
        if (count == goodsSaleProjectConfigList.size()) {
          goodsSaleProjectConfigList.forEach(t -> t.setCurrentIndex(1L));
        }
      });
      result.compute(
          projectConfigList.stream().map(t -> apsProjectConfigMap.get(t).getSaleCode()).sorted()
              .collect(Collectors.joining(",")),
...
// 以下为私有对象封装
  ...
  public @Override void setName(
      List<? extends ApsGoodsSaleProjectConfigDto> apsGoodsSaleProjectConfigDtoList) {
  ...
  private MPJLambdaWrapper<ApsGoodsSaleProjectConfig> getWrapper(ApsGoodsSaleProjectConfigDto obj) {
    ...
    if (Objects.nonNull(obj)) {
      q.eq(Objects.nonNull(obj.getGoodsId()), ApsGoodsSaleProjectConfig::getGoodsId,
              obj.getGoodsId())
          .eq(Objects.nonNull(obj.getFactoryId()), ApsGoodsSaleProjectConfig::getFactoryId,
              obj.getFactoryId())
          .eq(Objects.nonNull(obj.getProjectConfigId()),
              ApsGoodsSaleProjectConfig::getProjectConfigId, obj.getProjectConfigId())
          .eq(StringUtils.isNoneBlank(obj.getProjectConfigName()),
              ApsGoodsSaleProjectConfig::getProjectConfigName, obj.getProjectConfigName())
          .eq(Objects.nonNull(obj.getProjectConfigParentId()),
              ApsGoodsSaleProjectConfig::getProjectConfigParentId, obj.getProjectConfigParentId())
          .eq(Objects.nonNull(obj.getQuantity()), ApsGoodsSaleProjectConfig::getQuantity,
              obj.getQuantity())
          .eq(Objects.nonNull(obj.getSaleConfigId()), ApsGoodsSaleProjectConfig::getSaleConfigId,
              obj.getSaleConfigId())
          .eq(StringUtils.isNoneBlank(obj.getSaleConfigName()),
              ApsGoodsSaleProjectConfig::getSaleConfigName, obj.getSaleConfigName())
          .eq(Objects.nonNull(obj.getSaleConfigParentId()),
              ApsGoodsSaleProjectConfig::getSaleConfigParentId, obj.getSaleConfigParentId())
