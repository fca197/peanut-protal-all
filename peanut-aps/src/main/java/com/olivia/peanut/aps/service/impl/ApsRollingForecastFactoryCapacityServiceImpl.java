...
@Transactional
public class ApsRollingForecastFactoryCapacityServiceImpl extends
  ...
  public @Override ApsRollingForecastFactoryCapacityQueryListRes queryList(ApsRollingForecastFactoryCapacityQueryListReq req) {
    ...
    List<ApsRollingForecastFactoryCapacityDto> dataList = list.stream()
        .map(t -> $.copy(t, ApsRollingForecastFactoryCapacityDto.class))
  ...
  public @Override DynamicsPage<ApsRollingForecastFactoryCapacityExportQueryPageListInfoRes> queryPageList(ApsRollingForecastFactoryCapacityExportQueryPageListReq req) {
    ...
    if (Boolean.TRUE.equals(req.getQueryPage())) {
      ...
      IPage<ApsRollingForecastFactoryCapacityExportQueryPageListInfoRes> dataList = list.convert(
          t -> $.copy(t, ApsRollingForecastFactoryCapacityExportQueryPageListInfoRes.class));
      records = dataList.getRecords();
    } else {
      records = $.copyList(this.list(q),
          ApsRollingForecastFactoryCapacityExportQueryPageListInfoRes.class);
    }

    // 类型转换，  更换枚举 等操作

    List<ApsRollingForecastFactoryCapacityExportQueryPageListInfoRes> listInfoRes = $.copyList(
        records, ApsRollingForecastFactoryCapacityExportQueryPageListInfoRes.class);
...
//    req.setCapacityList(req.getCapacityList().stream().sorted(Comparator.comparing(ApsRollingForecastFactoryCapacityDtoInfo::getBeginTime)).toList());
    ...
    List<LocalDate> minMaxDate = DateUtils.getMinMaxDate(
        req.getCapacityList().stream().map(t -> List.of(t.getBeginTime(), t.getEndTime()))
            .toList());
    ...
    List<ApsRollingForecastFactoryCapacity> factoryCapacityList = this.list(
        new LambdaQueryWrapper<ApsRollingForecastFactoryCapacity>()//
            .eq(ApsRollingForecastFactoryCapacity::getFactoryId, req.getFactoryId())//
            .and(r -> {
              monthList.forEach((ym) -> {
                r.or(rt -> {
                  rt.eq(ApsRollingForecastFactoryCapacity::getYear, ym.getYear())
                      .eq(ApsRollingForecastFactoryCapacity::getMonth, ym.getMonth());
                });
              });
            }));
    ...
    req.getCapacityList().forEach(t -> {
      List<LocalDate> localDateBetween = DateUtils.getLocalDateBetween(t.getBeginTime(),
          t.getEndTime());
      localDateBetween.forEach(t1 -> {
        ...
        if (Objects.isNull(apsRollingForecastFactoryCapacity)) {
          ...
          apsRollingForecastFactoryCapacity.setFactoryId(req.getFactoryId()).setYear(t1.getYear())
              .setMonth(t1.getMonthValue());
        }
        ReflectUtil.setFieldValue(apsRollingForecastFactoryCapacity,
            "day" + ((t1.getDayOfMonth() < 10) ? "0" + t1.getDayOfMonth() : t1.getDayOfMonth()),
            t.getCapacity());
    ...
    if (CollUtil.isNotEmpty(values)) {
      List<ApsRollingForecastFactoryCapacity> insertList = values.stream()
          .filter(t -> Objects.isNull(t.getId())).toList();
      this.saveBatch(insertList);
      List<ApsRollingForecastFactoryCapacity> updateList = values.stream()
  ...
  public List<FactoryCapacityDay> list(Long factoryId, LocalDate beginDate, LocalDate endDate) {
    ...
    List<ApsRollingForecastFactoryCapacity> factoryCapacityList = this.list(
        new LambdaQueryWrapper<ApsRollingForecastFactoryCapacity>()//
            .eq(ApsRollingForecastFactoryCapacity::getFactoryId, factoryId).and(r -> {
              monthList.forEach(t -> {
                r.or(rt -> {
                  rt.eq(ApsRollingForecastFactoryCapacity::getYear, t.getYear())
                      .eq(ApsRollingForecastFactoryCapacity::getMonth, t.getMonth());
                });
              });
            }));
    ...
    Map<String, ApsRollingForecastFactoryCapacity> capacityMap = factoryCapacityList.stream()
    ...
    localDateBetween.forEach(t -> {
      ApsRollingForecastFactoryCapacity apsRollingForecastFactoryCapacity = capacityMap.get(
          t.getYear() + "-" + t.getMonthValue());
      if (Objects.nonNull(apsRollingForecastFactoryCapacity)) {
        String fieldName =
            "day" + ((t.getDayOfMonth() < 10) ? "0" + t.getDayOfMonth() : t.getDayOfMonth());
        Field field = getField(apsRollingForecastFactoryCapacity, fieldName);
        Integer capacity = FieldUtils.getFieldValue(apsRollingForecastFactoryCapacity,
            field);
